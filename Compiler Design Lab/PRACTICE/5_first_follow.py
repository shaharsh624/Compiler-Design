            ...
            for sub_symbol in production:
                if sub_symbol in terminals:
                    if prev_symbol is not None:
                        calculate_first_recursive(
                            grammar, prev_symbol, first_sets, terminals
                        )
                        if epsilon in first_sets[prev_symbol]:
                            first_sets[symbol] = first_sets[symbol].union(
                                first_sets[prev_symbol]
                            )
                    first_sets[symbol].add(sub_symbol)
                    break
                elif sub_symbol != epsilon:
                    calculate_first_recursive(
                        grammar, sub_symbol, first_sets, terminals
                    )
                    first_sets[symbol] = first_sets[symbol].union(
                        first_sets[sub_symbol]
                    )
