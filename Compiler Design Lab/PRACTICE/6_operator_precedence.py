...
def stringcheck():
    ...
    i = list(
        input(
            "Enter the string want to be checked(non-terminals should be in (small letters) :: "
        )
    )
    i.append("$")
    s = [None] * len(i)
    ...
    while s[0] != s[1]:
        if i[len(i) - 2] in p:
            ...
            if n[x.index(s[q])][y.index(i[h])] == "<":
                ...
                s.pop(q)
                q -= 1
            elif (n[x.index(s[q])][y.index(i[h])] == "") and (
                (s[q] == "$") and (i[h] == "$")
            ):
                s[1] = s[0]
        else:
    ...
    if b[0] == " " or b[0] == "" or b[0] in f or len(b) == 1:
        ...
        for i in range(0, len(b), 2):
            if b[i] == " ":
                ...
                g = False
                break
            elif b[len(b) - 1] in o and (
                (b[0] == "(" and b[len(b) - 1] == ")") or (b.count("(") == b.count(")"))
            ):
                g = True
            elif b[i] in f:
                g = True
                ...
                (i == len(b) - 1) and (b[i] not in s) and (b[i] in o) and b[i - 1] in o
            ):
                g = True
